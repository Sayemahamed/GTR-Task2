...
class Device(SQLModel, table=True):
    ...
    model_name: str = Field(
        sa_column=Column("model_name", String, primary_key=True, index=True, nullable=False)
    )
    ...
    ram_gb: int = Field(
        ge=0, description="RAM size in gigabytes (GB)."
    )
    ...
    price_cents: Optional[int] = Field(
        default=None, ge=0, description="Price in the smallest currency unit (e.g., cents)."
    )